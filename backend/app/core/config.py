...
class DatabaseConfig:
    ...
    def get_url() -> str:
        host = os.getenv("DB_HOST", "localhost")
        port = os.getenv("DB_PORT", "5432")
        name = os.getenv("DB_NAME", "signature_communiques_officiels")
        user = os.getenv("DB_USER", "magistral")
        password = os.getenv("DB_PASSWORD", "")
        return f"postgresql://{user}:{password}@{host}:{port}/{name}"
    ...
    @staticmethod
    def get_async_url() -> str:
        return DatabaseConfig.get_url().replace("postgresql://", "postgresql+asyncpg://")
...
# Instance unique — APRÈS la définition de la classe
settings = Settings()