...
import {
  ...
  User, LogOut, Menu, X, Settings, FolderOpen, Bell, ChevronDown,
} from "lucide-react";
...
export const MainLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) => [
    ...
    isActive
      ...
      : "text-gray-600 dark:text-dark-300 hover:text-gray-900 dark:hover:text-dark-100 hover:bg-gray-100 dark:hover:bg-dark-600",
  ].join(" ");
    ...
    <aside className="flex flex-col h-full bg-white dark:bg-dark-800">
      ...
      <nav className="flex-1 px-3 pb-4 space-y-0.5 overflow-y-auto">
        {visibleItems.map(item => (
          <NavLink key={item.to} to={item.to} end={item.to === "/dashboard"}
            className={navLinkClass} onClick={() => setSidebarOpen(false)}>
      ...
      <div className="px-3 pb-4 space-y-0.5 border-t border-gray-100 dark:border-dark-600 pt-3">
        ...
        <button onClick={handleLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/30 transition-all duration-150">
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-dark-900 transition-colors duration-200 overflow-hidden">
      {/* Desktop sidebar */}
      <div className="hidden md:flex flex-col w-64 shrink-0 border-r border-gray-100 dark:border-dark-600">
        <Sidebar />
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-14 shrink-0 flex items-center justify-between px-4 bg-white dark:bg-dark-800 border-b border-gray-100 dark:border-dark-600 gap-3">
          <div className="flex items-center gap-3">
            <button className="md:hidden p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 dark:text-dark-300 dark:hover:bg-dark-700 transition-colors"
              onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Menu">
            ...
            <div className="flex md:hidden items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-900 dark:text-dark-100 tracking-widest">SHIELD</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-xl text-gray-400 dark:text-dark-400 hover:bg-gray-100 dark:hover:bg-dark-700 transition-colors">
              <Bell size={18} />
            </button>
            <ThemeToggle />
            {/* User menu */}
            <div className="relative">
              <button onClick={() => setUserMenuOpen(o => !o)}
                className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl hover:bg-gray-50 dark:hover:bg-dark-700 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-dark-500">
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 z-20 w-52 rounded-xl bg-white dark:bg-dark-700 border border-gray-100 dark:border-dark-500 shadow-lg overflow-hidden">
                    ...
                    <div className="p-1.5">
                      <button onClick={() => { setUserMenuOpen(false); navigate("/dashboard/profile"); }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium text-gray-700 dark:text-dark-200 hover:bg-gray-50 dark:hover:bg-dark-600 transition-colors">
                        <User size={14} />Mon profil
                      </button>
                      <button onClick={() => { setUserMenuOpen(false); navigate("/dashboard/settings"); }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium text-gray-700 dark:text-dark-200 hover:bg-gray-50 dark:hover:bg-dark-600 transition-colors">
