// pages/dashboard/VerifyDocument.tsx — v3
...
const DocumentInfoCard: React.FC<{ info: VerifyResult["document_info"] }> = ({ info }) => (
  <div className="bg-white dark:bg-dark-800 rounded-2xl border border-gray-100 dark:border-dark-600 p-5">
    <div className="flex items-center gap-2 mb-3">
      ...
      <h3 className="text-sm font-bold text-gray-700 dark:text-dark-200 uppercase tracking-wide">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
      {[
        ...
        { label: "Date de signature", value: info.date_signature
            ? new Date(info.date_signature).toLocaleString("fr-FR", {
...
const NiveauCard: React.FC<{
  numero: 1|2|3; label: string; niveau: Niveau; defaultOpen?: boolean;
}> = ({ numero, label, niveau, defaultOpen = false }) => {
  ...
  if (!niveau.execute) return (
    <div className="rounded-xl border border-gray-100 dark:border-dark-600 bg-gray-50 dark:bg-dark-800 p-4 opacity-40">
      <div className="flex items-center gap-3">
        <span className="w-7 h-7 rounded-full bg-gray-200 dark:bg-dark-600 flex items-center justify-center text-xs font-bold text-gray-500">
          {numero}
        </span>
  ...
  const borderColor = niveau.valide
    ? "border-emerald-200 dark:border-emerald-800"
    : "border-red-200 dark:border-red-800";
  const bgColor = niveau.valide
  ...
  return (
    <div className={`rounded-xl border ${borderColor} ${bgColor} overflow-hidden`}>
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-black/5 dark:hover:bg-white/5 transition"
      >
        <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${niveau.valide ? "bg-emerald-500" : "bg-red-500"}`}>
          {numero}
        </span>
          ...
          {niveau.ocr_info && (
            <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-dark-400">
              <span className={`inline-flex items-center gap-1 ${
                niveau.ocr_info.zone_qr_masquee
                ...
                <div className="h-2.5 rounded-full bg-gray-200 dark:bg-dark-600 overflow-hidden">
                  <div
                    className={`h-full rounded-full ${
                      diff.similarite_pct >= 98 ? "bg-emerald-500" :
                      diff.similarite_pct >= 90 ? "bg-amber-500" : "bg-red-500"
                    }`}
                    style={{ width: `${diff.similarite_pct}%` }}
                  />
                </div>
                {diff.anomalies.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-dark-400">
                          ...
                          <div className="flex gap-1 shrink-0">
                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                              a.gravite === "haute"
                                ...
                                : "bg-gray-200 text-gray-700 dark:bg-dark-600 dark:text-dark-300"
                            }`}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {a.original && (
                            <div className="bg-red-50 dark:bg-red-950/20 rounded p-1.5">
                              <div className="text-[10px] text-red-600 dark:text-red-400 font-semibold mb-0.5">Original</div>
                              <code className="font-mono break-all">{a.original}</code>
                            </div>
                          )}
                          {a.scanned && (
                            <div className="bg-blue-50 dark:bg-blue-950/20 rounded p-1.5">
                              <div className="text-[10px] text-blue-600 dark:text-blue-400 font-semibold mb-0.5">Scanné</div>
                              <code className="font-mono break-all">{a.scanned}</code>
                            </div>
                          )}
                        </div>
                        {a.contexte && (
                    ...
                    {diff.anomalies.length > 5 && (
                      <button
                        ...
                        className="w-full py-2 rounded-xl border border-gray-200 dark:border-dark-500 text-xs font-medium text-gray-600 dark:text-dark-300 hover:bg-gray-50 dark:hover:bg-dark-700 transition flex items-center justify-center gap-1"
                      >
                        {showAll
                          ? <><EyeOff size={12} />Réduire</>
                          : <><Eye size={12} />Voir toutes les {diff.anomalies.length} anomalies</>
                        }
                      </button>
                    )}
                  </div>
  ...
  if (step === "input") return (
    <AuthLayout>
      <div className="w-full max-w-md space-y-5">
        ...
        <div className="bg-white dark:bg-dark-800 rounded-2xl shadow-xl border border-gray-100 dark:border-dark-600 p-6 space-y-5">
          {error && (
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all ${
              dragOver
                ? "border-army-500 bg-army-50 dark:bg-army-950/20 scale-[1.02]"
                : file
  ...
  if (step === "loading") return (
    <AuthLayout>
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-dark-800 rounded-2xl shadow-xl border border-gray-100 dark:border-dark-600 p-10 flex flex-col items-center gap-5">
    ...
    <AuthLayout>
      <div className="w-full max-w-2xl space-y-5 py-6">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900 dark:text-dark-100 flex items-center gap-2">
            <ShieldCheck size={20} className="text-army-600" />Résultat de vérification
          </h1>
          <button
            onClick={handleReset}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-dark-500 text-sm font-medium text-gray-600 dark:text-dark-300 hover:bg-gray-50 dark:hover:bg-dark-700 transition"
          >
        ...
        <div className="space-y-3">
          <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-dark-400">
            Détail par niveau
          </h2>
          ...
          <NiveauCard numero={3} label="Niveau 3 — Analyse textuelle fine"                niveau={result.niveau3} defaultOpen={result.niveau3.execute} />
        </div>

        <div className="bg-white/80 dark:bg-dark-800/80 rounded-xl border border-gray-100 dark:border-dark-600 p-4 text-xs text-gray-500 dark:text-dark-400">
          <p className="font-semibold mb-1">ℹ️ Note sur la vérification</p>
          <p>
            Ce système utilise la cryptographie RSA-PSS SHA-256. Une vérification de Niveau 1 valide
            garantit que l'agent officiel a bien signé ce document. Le Niveau 3 peut présenter des
            faux positifs si le scan est de mauvaise qualité — dans ce cas, consultez le document
            original sur SHIELD.
          </p>
        </div>
        ...
        <button
          onClick={handleReset}
          className="w-full py-3 rounded-2xl border-2 border-army-200 dark:border-army-800 text-army-700 dark:text-army-400 font-semibold text-sm hover:bg-army-50 dark:hover:bg-army-950/20 transition flex items-center justify-center gap-2"
        >
